...
const RejectedProperties = () => {
  ...
  const fetchRejectedProperties = async () => {
    try {
      ...
      const response = await propertyApi.getRejectedProperties();
    ...
    <div className="min-h-screen bg-black">
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-2xl font-bold text-white">Rejected Properties</h1>
        </div>

        {properties.length === 0 ? (
          <div className="text-white text-center py-8">No rejected properties found</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {properties.map((property) => (
              <div
                key={property.id}
                className="bg-black/80 rounded-xl overflow-hidden border border-red-600 hover:shadow-red-500/20 transition-shadow duration-300"
              >
                {/* Image Carousel */}
